...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Jikeradio icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Jikeradio';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Jikeradio executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Jikeradio icon was not found: ${iconPath}`);
  ...
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Jikeradio',
    '--set-version-string', 'ProductName', 'Jikeradio',
    '--set-version-string', 'CompanyName', 'Jikeradio',
